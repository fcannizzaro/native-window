...
| Build addon (release)    | `bun run build`          | `packages/native-window`    |
| Build addon (debug)      | `bun run build:debug`    | `packages/native-window`    |
| Format Rust              | `cargo fmt`              | `packages/native-window`    |
| Lint Rust                | `cargo clippy`           | `packages/native-window`    |
| Run all tests            | `vitest run`             | `packages/native-window-ipc`|
| Run single test by name  | `vitest run -t "pattern"`| `packages/native-window-ipc`|
| Run sample               | `bun samples/src/basic.ts` | repo root                 |

## Testing

- Framework: `vitest`. Uses `describe`/`test`/`expect`/`mock`.
- Tests live in `packages/native-window-ipc/tests/channel.test.ts`.
...
1. Implement in `packages/native-window-ipc/index.ts` (host) and/or `client.ts` (webview).
