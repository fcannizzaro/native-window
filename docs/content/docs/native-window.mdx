...
- **HTML origin on Windows** — content loaded via `loadHtml()` has an `about:blank` origin on Windows (WebView2 limitation); on macOS, a synthetic `https://native-window.local/` origin is used. Use `loadUrl()` for fetch/XHR that requires a real origin.
