import type { NativeWindow, WindowOptions } from "@fcannizzaro/native-window";
...
export type InferOutput<S> =
  S extends { _zod: { output: infer T } } ? T :
  S extends { _types?: { output: infer T } } ? T :
 ...
 * @internal
 */
export type SendArgs<T extends EventMap, K extends keyof T & string> =
  [T[K]] extends [void | never]
...
export interface NativeWindowChannel<T extends EventMap>
  extends TypedChannel<T> {
...
export function createChannel<S extends SchemaMap>(
  ...
  win.onMessage((raw: string, sourceUrl: string) => {
    ...
    for (const fn of set) { try { fn(validatedPayload); } catch {} }
  });
...
export function createWindow<S extends SchemaMap>(
  ...
  // Import NativeWindow from the native-window package
  const { NativeWindow: NW } = require("@fcannizzaro/native-window") as typeof import("@fcannizzaro/native-window");
  const win = new NW(windowOptions);
