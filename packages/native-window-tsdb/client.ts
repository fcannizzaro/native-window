 ...
 * ```
 */
export interface NativeWindowCollectionConfig<
  T extends object,
...
interface SyncParams<T extends object, TKey extends string | number> {
  ...
  write: (
    message:
      | { value: T; type: "insert" | "update" | "delete"; previousValue?: T; metadata?: Record<string, unknown> }
 ...
 */
export interface NativeWindowCollectionResult<
  T extends object,
  TKey extends string | number,
> {
  id: string;
...
>(
  config: NativeWindowCollectionConfig<T, TKey>,
): NativeWindowCollectionResult<T, TKey> {
  ...
  return {
    ...
    sync: {
      sync: (params: SyncParams<T, TKey>): (() => void) => {
        ...
        const prev = (window as any).__native_message__ as
          | ((msg: string) => void)
          | undefined;
