 ...
 * `@fcannizzaro/native-window-ipc/client` for use inside a React app
...
export type {
  ...
  TypedChannel,
} from "@fcannizzaro/native-window-ipc";

export type {
  ChannelClientOptions,
} from "@fcannizzaro/native-window-ipc/client";
 ...
 * `createChannelClient` from `@fcannizzaro/native-window-ipc/client`.
 * The channel instance is stable for the lifetime of the provider.
 *
 * @example
 * ```tsx
 * import { z } from "zod";
 * import { ChannelProvider } from "@fcannizzaro/native-window-ipc-react";
 *
 * const schemas = {
 *   counter: z.number(),
 *   title: z.string(),
 ...
 * ```
 */
export function ChannelProvider<S extends SchemaMap>(
  props: ChannelProviderProps<S>,
): ReactNode {
 ...
 * @example
 * ```tsx
 * import { useChannel } from "@fcannizzaro/native-window-ipc-react";
 *
...
export function useChannel<T extends EventMap = EventMap>(): TypedChannel<T> {
  ...
  if (channel === null) {
    throw new Error(
      "useChannel() must be used inside a <ChannelProvider>.",
    );
  }
 ...
 * @example
 * ```tsx
 * import { useChannelEvent } from "@fcannizzaro/native-window-ipc-react";
 *
 ...
 * @example
 * ```tsx
 * import { useSend } from "@fcannizzaro/native-window-ipc-react";
 *
 * type Events = { counter: number; title: string };
 *
 * function Counter() {
...
export interface TypedChannelHooks<T extends EventMap> {
  ...
  useChannelEvent: <K extends keyof T & string>(
    type: K,
    handler: (payload: T[K]) => void,
  ) => void;
  /** Returns a stable typed `send` function. */
  useSend: () => <K extends keyof T & string>(
    ...args: SendArgs<T, K>
  ) => void;
}
...
export function createChannelHooks<S extends SchemaMap>(
  ...
  function hooks_useSend(): <K extends keyof T & string>(
    ...args: SendArgs<T, K>
  ) => void {
    const channel = hooks_useChannel();
