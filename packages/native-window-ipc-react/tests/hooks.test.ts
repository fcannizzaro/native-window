...
import type { TypedChannel } from "@fcannizzaro/native-window-ipc";
...
vi.mock("@fcannizzaro/native-window-ipc/client", () => ({
...
describe("useChannelEvent", () => {
  ...
  test("calls latest handler without re-subscribing", () => {
    ...
    const { rerender } = renderHook(
      ({ handler }) => useChannelEvent("ping", handler),
...
describe("createChannelHooks", () => {
  ...
  test("useChannelEvent subscribes and unsubscribes", () => {
    ...
    const { unmount } = renderHook(
      () => hooks.useChannelEvent("ping", handler),
      { wrapper: Wrapper },
    );
